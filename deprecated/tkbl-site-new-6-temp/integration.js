...
(function (window, document, JSON, Object) {
  ...
  var talkable = (window.talkable = (function () {
    ...
    methods = {
      ...
      show_claim_by_name: function (data) {
        ...
        if (matchedPlacementIds !== EMPTY_PLACEMENTS) {
          ...
          utils.log('No name sharing placements matched.');
        }
      },
    ...
    return {
      ...
      run: function () {talkable.check(),talkable._timer=setInterval(talkable.check,config.queue_check_interval),talkable._launch_campaigns_retry=setInterval((function(){utils.launchCampaignsCriteria()&&(clearInterval(talkable._launch_campaigns_retry),utils.launchCampaigns())}),config.launch_campaigns_retry_delay),setTimeout((function(){clearInterval(talkable._launch_campaigns_retry)}),config.launch_campaigns_retry_count*config.launch_campaigns_retry_delay)},_timer:null,_launch_campaigns_retry:null});_TALKABLE_PER_CLIENT_CONFIG={server:"https://www.bastion.talkable.com",site_slug:"testmax-new-site-12dec2022"},window._talkableq=window._talkableq||[],_talkableq.unshift(["init",{site_id:_TALKABLE_PER_CLIENT_CONFIG.site_slug,server:_TALKABLE_PER_CLIENT_CONFIG.server,integration_platform:""}]),talkable.run()}(window,document,JSON,Object);
                       //# sourceMappingURL=talkable-5.1.0.min.map